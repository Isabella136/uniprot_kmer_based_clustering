...
pub struct KmerEdgeHelper {
    ...
    array_0: [ArcUsize; 4000],
    array_1: [ArcUsize; 4000],

    // bit-array informing whether other threads have populated vertex key array
    bit_array_0: [ArcBool; 4000],
    bit_array_1: [ArcBool; 4000],
...
impl KmerEdgeHelper {
    pub fn new() -> KmerEdgeHelper {
        KmerEdgeHelper { 
            ...
            array_0: [0usize; 4000].map(|x| Arc::new(AtomicUsize::new(x))),
            array_1: [0usize; 4000].map(|x| Arc::new(AtomicUsize::new(x))),
            bit_array_0: [false; 4000].map(|x| Arc::new(AtomicBool::new(x))),
            bit_array_1: [false; 4000].map(|x| Arc::new(AtomicBool::new(x))),
...
impl KmerEdgeSingle {
    ...
    // Make a new KmerEdge
    fn new(prefix_sum: ArcUsize, offset: usize, 
            vertices_count: usize) -> KmerEdgeSingle {
        KmerEdgeSingle {
            kmer: (prefix_sum, offset),
            vertices_key: vec![],
...
impl KmerEdge {
    pub fn get_kmers(&self) -> Vec<usize> {
        match self {
            Self::Single(val) => vec![val.kmer.0.load(Acquire) + val.kmer.1],
    ...
    pub fn get_vertices_bit_array(&self) -> Vec<ArcBool> {
        match self {
    ...
    pub fn new(prefix_sum: ArcUsize, offset: usize,
            vertices_count: usize) -> KmerEdge {
        Self::Single(KmerEdgeSingle::new(prefix_sum, offset, vertices_count))
    }
